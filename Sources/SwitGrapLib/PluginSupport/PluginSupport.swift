...
class PluginSupport {
    ...
    func generateDigraph(target: String, projectSourceFiles: [FileManager.Path]) throws -> Digraph {
        ...
        for file in projectSourceFiles {
            let pluginFile = SwitGrapherFile(
        ...
        for module in targetImports {
            ...
            // (only Apple frameworks imported by the main --target)
                ...
                let _nodes = try plugin_process(library: SwitGrapherImport(moduleName: module, importerName: target, moduleType: .apple, importerType: .target))
                nodes.append(contentsOf: _nodes)
            }

            // Weird unknown cases
            else if unknownManager?.isManaging(module: module) == true {
                let _nodes = try plugin_process(library: SwitGrapherImport(moduleName: module, importerName: target, moduleType: .other, importerType: .target))
                nodes.append(contentsOf: _nodes)
            }
        }
...
private extension PluginSupport {
    ...
    func recurseSwiftPackages(from module: String, importedBy importer: String, importerType: SwitGrapherImport.ModuleType, building nodeList: inout [Any], skipping modulesToSkip: inout Set<String>) throws {
        if swiftPackageManager?.isManaging(module: module) == true {
            // `module` is a Swift Package and `importer` is either a Swift Package or the main --target
            let _nodes = try plugin_process(library: SwitGrapherImport(moduleName: module, importerName: importer, moduleType: .spm, importerType: importerType))
            ...
            let _nodes = try plugin_process(library: SwitGrapherImport(moduleName: module, importerName: importer, moduleType: .apple, importerType: importerType))
            nodeList.append(contentsOf: _nodes)
        } else if unknownManager?.isManaging(module: module) == true {
            modulesToSkip.insert(module)

            // Weird case
            let _nodes = try plugin_process(library: SwitGrapherImport(moduleName: module, importerName: importer, moduleType: .other, importerType: importerType))
            nodeList.append(contentsOf: _nodes)
        }
    }

    func recurseCocoapods(from module: String, importedBy importer: String, importerType: SwitGrapherImport.ModuleType, building nodeList: inout [Any], skipping modulesToSkip: inout Set<String>) throws {
        let _nodes = try plugin_process(library: SwitGrapherImport(moduleName: module, importerName: importer, moduleType: .cocoapods, importerType: importerType))
        nodeList.append(contentsOf: _nodes)
...
private extension PluginSupport {
    ...
    func plugin_process(library: SwitGrapherImport) throws -> [Any] {
    ...
    func plugin_process(file: SwitGrapherFile) throws -> [Any] {
        do {
    ...
    func plugin_makeArrows(from processingResults: [Any]) throws -> [SwitGrapherArrow] {
        do {
